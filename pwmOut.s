...
tccrsetup:
	...
	sts TCCR1A , r16
	ldi r16, 0b00011001 ;ICP noise canceler, prescaler, edge selector set to zero/falling edge triggers event.
	sts TCCR1B , r16
	ldi r23 , 0x6D
	ldi r24 , 0xA1
...
acompsetup: 
	...
	sts PORTD, r16
	sts DDRD, r16
	ldi r16, (0<<ACD)|(0<<ACBG)|(1<<ACIE)|(0<<ACIC)|(1<<ACIS1)|(1<<ACIS0);tcnt input capture disabled, analog comp interrupt enabled, ACIS10 dont matter
	sts ACSR , r16
...
ACOMP: ;called when Analog comparatoris toggled which means weve hit our target value
	...
	nop
	pop r16
	out SREG , r16 ; Recover the status register from the stack	reti;
TIM1_CAPT: ;disabled
	nop
	reti
TIM1_COMPA:
