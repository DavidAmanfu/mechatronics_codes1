...
acompsetup: 
	...
	sts PORTD, r16
	ldi r16, (0<<ACD)|(0<<ACBG)|(1<<ACIE)|(0<<ACIC)|(1<<ACIS1)|(0<<ACIS0);enabled, no bandgap, enabled interruptor, disabled input capture, but this might still be necessary
 	sts ACSR , r16
...
TCNTInterruptSetup:
	...
	ldi r16 , (0<<ICIE1)|(0<<OCIE1B)|(0<<OCIE1A)|(1<<TOIE1); oca,b
	sts TIMSK1, r16
	ret
interruptsetup:
	...
	ldi r16 , (0<<INT1MSK)|(0<<INT0MSK) ; digital pins 2 = 0 and 3
...
ACOMP: ;falling edge, angle is now too big
	in r16 , SREG ; Save the current status register to stack
	push r16
...
TIM1_OVF:
	...
	out SREG , r16 ; Recover the status register from the stack
	reti
ADC: ;rising edge of comp, angle too small
	in r16 , SREG ; Save the current status register to stack
	push r16
