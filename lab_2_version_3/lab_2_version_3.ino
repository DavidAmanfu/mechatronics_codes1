...
int dibsOnFirst = 0;        //is it noise? 0 for yes 1 for no
 ...
 // int sensorZValue = analogRead(A0); //read the input on analog pin 0:
  ...
  if(irValue<=irsensorMax && irValue>=irsensorMin){
    ...
    if(dibsOnSecond==1){ //if you've gone through the code and now are reading noise, and you've already hit the first value
      ...
      int dibsOnSecond = 2; //update the counter
    }
    else if(dibsOnSecond==3){
    ...
    if(dibsOnSecond==0){
      ...
      int dibsOnSecond = 1; //you've hit the first line
      }
    else if(dibsOnSecond == 2){ //you're no longer reading noise (First=1) and you've hit the counter twice for time stamps
      Serial.print("second hit ");Serial.println(millis()); //print that out
      int dibsOnSecond=3; //update the counter
      }
  }  //you're reading a value
  
  //  if(sensorZValue>=sensorMax || sensorZValue<=sensorMin){
  //    if(dibsOnSecond=4){Serial.print("table hit");Serial.println(millis());}
  //}
